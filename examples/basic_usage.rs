...
async fn main() -> Result<()> {
    ...
    println!("🔧 Engine Status: Ready={}, Uptime={}s", 
        engine_status.is_ready, 
        engine_status.uptime_seconds
    );
    ...
    match engine.send_message("", "test").await {
        ...
        Err(e) => {
            match e {
                ...
                whatsapp_engine::WhatsAppError::Timeout { operation, timeout_seconds } => {
                    println!("Operation '{}' timed out after {}s", operation, timeout_seconds);
                }
                _ => {
