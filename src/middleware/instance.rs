...
pub fn check_instance_access(
    ...
    // Get user's permission for this instance
    let permission = match db.get_instance_permission(user_id, instance_id) {
        Ok(Some(perm)) => perm,
        Ok(None) => {
            return Err((
                StatusCode::FORBIDDEN,
                Json(json!({
                    "error": "forbidden",
                    "message": format!("Access denied - you don't have access to instance '{}'", instance_id),
        ...
        Err(e) => {
            tracing::error!("Failed to check instance permission: {}", e);
            return Err((
    ...
    if auth_user.can_access_instance(Some(permission), required_permission) {
        ...
        Err((
            ...
            Json(json!({
                "error": "forbidden",
                "message": format!(
                    "Insufficient permissions - you have '{}' but '{}' is required",
                    permission, required_permission
                ),
                "instance_id": instance_id,
                "your_permission": permission.to_string(),
