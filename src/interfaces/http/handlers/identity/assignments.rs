...
pub async fn get_user_instances(
    ...
    if !auth_user.is_admin() && !is_self {
        return (
            ...
            Json(json!({"error":"forbidden","message":"Admin access or self-access required"})),
        )
            .into_response();
    }
...
pub async fn assign_instance(
    ...
    match db.get_instance(&req.instance_id) {
        Ok(None) => {
            return (
                StatusCode::NOT_FOUND,
                Json(json!({"error":"not_found","message":"Instance not found"})),
            )
                .into_response()
        }
        Err(e) => {
    ...
    match db.assign_instance_to_user(&req.user_id, &req.instance_id, req.permission) {
        ...
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error":"assign_failed","message":e.to_string()})),
        )
            .into_response(),
    }
}
...
pub async fn remove_instance(
    ...
    match db.remove_instance_from_user(&user_id, &instance_id) {
        Ok(()) => (
            StatusCode::OK,
            Json(json!({"message":"Instance removed from user"})),
        )
            .into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({"error":"remove_failed","message":e.to_string()})),
        )
            .into_response(),
    }
}
...
mod tests {
    use super::*;
    #[test]
    fn test_assign_handlers_exist() {
        let _ = std::any::type_name::<AssignInstanceRequest>();
    }
}
