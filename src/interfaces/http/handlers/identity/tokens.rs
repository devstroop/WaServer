    ...
    match db.create_access_token(&token_id, &user_id, &req.name, &hash, expires_at.as_deref()) {
        Ok(rec) => (
            ...
            Json(CreateAccessTokenResponse {
                token_info: AccessTokenInfo::from(rec),
                access_token: token,
            }),
        )
            .into_response(),
...
pub async fn list_access_tokens(
    ...
    match db.list_user_access_tokens(&user_id) {
        Ok(tokens) => {
            let infos: Vec<AccessTokenInfo> =
                tokens.into_iter().map(AccessTokenInfo::from).collect();
            (
                StatusCode::OK,
                Json(ListAccessTokensResponse { tokens: infos }),
            )
                .into_response()
        }
        Err(e) => (
...
pub async fn delete_access_token(
    ...
    match db.delete_access_token(&token_id, &user_id) {
        Ok(()) => (
            StatusCode::OK,
            Json(json!({"message":"Access token deleted successfully"})),
        )
            .into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
mod tests {
    use super::*;
    #[test]
    fn test_token_handlers_exist() {
        let _ = std::any::type_name::<CreateAccessTokenRequest>();
    }
}
