...
impl SessionManager {
    ...
    pub async fn save_session(&mut self, session_data: SessionData) -> Result<()> {
        ...
        let json_data = serde_json::to_string_pretty(&session_data).map_err(|e| {
            WhatsAppError::SerializationError {
    ...
    async fn load_session_from_file(&self, path: &PathBuf) -> Result<SessionData> {
        ...
        let session_data: SessionData =
            serde_json::from_str(&content).map_err(|e| WhatsAppError::SerializationError {
...
mod tests {
    ...
    async fn test_update_session_phone_number() {
        ...
        session_manager
            .update_session_phone_number(phone.clone())
            .await
            .unwrap();
