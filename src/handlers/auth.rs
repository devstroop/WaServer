use crate::{
    models::auth::{AuthStatusResponse, PhoneAuthResponse, QrCodeResponse, SuccessResponse, ErrorResponse},
...
pub async fn get_qr_code(
    ...
    match whatsapp_service.execute_with_busy_flag(async {
...
pub async fn login_with_phone(
    ...
    match whatsapp_service.execute_with_busy_flag(async {
        whatsapp_service.auth_service().login_with_phone_number(&phone_number).await
    }).await {
        Ok(code) => {
...
pub async fn logout(
    ...
    match whatsapp_service.execute_with_busy_flag(async {
        whatsapp_service.auth_service().logout().await
    }).await {
        Ok(_) => {
