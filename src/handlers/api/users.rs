...
pub async fn update_user(
    ...
    // Revoke web sessions on password change or deactivation (security #45)
    if password_hash_opt.is_some() {
        let _ = db.delete_user_web_sessions(&user_id);
    }
    if matches!(request.is_active, Some(false)) {
        let _ = db.delete_user_web_sessions(&user_id);
    }
