...
use wae_rust::{
    ...
    services::{
        ...
        improved_phone_auth::ImprovedPhoneAuthService,
    },
};
...
async fn test_phone_auth_integration() -> Result<()> {
    ...
    let result = auth_service.login_with_phone_number_improved("919501005734").await;
...
async fn test_standalone_improved_service() -> Result<()> {
    ...
    for step in expected_steps {
        assert!(result.debug_info.steps_completed.contains(&step.to_string()), 
                "Missing step: {}", step);
    }
