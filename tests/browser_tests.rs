...
mod browser_tests {
    ...
    async fn test_browser_initialization() -> Result<()> {
        ...
        match result {
            ...
            Err(e) => {
                println!(
                    "Browser initialization failed (expected if Chrome not available): {}",
                    e
                );
    ...
    async fn test_page_creation() -> Result<()> {
        ...
        if browser_service.initialize().await.is_ok() {
            ...
            match browser_service
                .get_or_create_page("https://example.com")
                .await
            {
                Ok(page) => {
    ...
    async fn test_browser_service_graceful_failure() -> Result<()> {
        ...
        config
            .browser
            .args
            .push("--invalid-argument-that-should-fail".to_string());
    ...
    async fn test_browser_service_whatsapp_specific() -> Result<()> {
        ...
        if browser_service.initialize().await.is_ok() {
            ...
            let page1_result = browser_service
                .get_or_create_page("https://web.whatsapp.com")
                .await;
            let page2_result = browser_service.get_whatsapp_page().await;
            let page3_result = browser_service
                .get_or_create_page("https://web.whatsapp.com/send?phone=1234567890")
                .await;
