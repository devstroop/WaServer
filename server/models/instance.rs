...
pub struct InstanceConfig {
    ...
    /// Instance name (user-defined label)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub instance_name: Option<String>,

    /// Idle timeout in seconds before the instance auto-sleeps (0 = never sleep)
...
pub struct CreateInstanceRequest {
    ...
    /// Instance name (user-defined label, defaults to "unknown")
...
/// WhatsApp instance status response (for /api/v1/account/{instance_id}/status)
#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub struct WhatsAppStatusResponse {
    ...
    /// Phone number in E.164 format (populated after WhatsApp login)
    pub phone_number: Option<String>,
    /// Instance status (sleeping, warming_up, active, error)
    pub status: String,
