...
export default function InstanceDetail() {
  ...
  const backTo = isAdminRoute ? '/admin/instances' : location.pathname.startsWith('/dashboard') ? '/dashboard/instances' : '/app/instances';
  ...
  useEffect(() => {
    if (status?.authorized) {
      setInstance((prev) => (prev ? { ...prev, authorized: true, status: status.status } : prev));
    }
  }, [status]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <Link to={backTo} className="rounded border bg-white px-3 py-1.5 text-sm hover:bg-zinc-50">
          ← Back
        </Link>
        <h1 className="text-xl font-semibold tracking-tight">
          {instance?.name ?? 'Instance'} <span className="font-mono text-sm font-normal text-zinc-500">{id.slice(0, 8)}</span>
        </h1>
        ...
        {loadingInstance ? (
          ...
          <div className="grid grid-cols-1 gap-3 text-sm md:grid-cols-3">
            ...
            <div>
              ...
              <div className="mt-1">
                <Badge tone={statusTone(status?.status ?? instance.status)}>{status?.status ?? instance.status}</Badge>
              </div>
            </div>
            <div>
